    ...
    <div className="max-w-7xl mx-auto space-y-8 px-4 font-sans pb-12">
      <div className="flex items-center gap-4 mt-8 mb-8">
        ...
        <div>
          ...
          <p className="text-gray-500 font-medium">Administrator Operations Center</p>
        </div>
      </div>
      ...
      <div className="flex gap-6 mb-8 border-b border-gray-100 pb-0">
        <button 
          ...
          className={`font-black uppercase tracking-widest text-sm pb-4 border-b-2 transition-all ${activeTab === 'disputes' ? 'border-gray-900 text-gray-900' : 'border-transparent text-gray-400 hover:text-gray-600 hover:border-gray-300'}`}
        >
          Dispute Resolution
        </button>
        <button 
          onClick={() => setActiveTab('users')} 
          className={`font-black uppercase tracking-widest text-sm pb-4 border-b-2 transition-all ${activeTab === 'users' ? 'border-gray-900 text-gray-900' : 'border-transparent text-gray-400 hover:text-gray-600 hover:border-gray-300'}`}
        >
          User Moderation
        </button>
      </div>
               ...
               <tbody>
                  {users.map(u => (
                    <tr key={u.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                      <td className="py-4 px-6 text-xs font-bold text-gray-400">#{u.id}</td>
                      ...
                      <td className="py-4 px-6 text-sm text-gray-500 font-medium">{u.email}</td>
                      <td className="py-4 px-6">
                        {u.is_banned ? (
                           <span className="bg-red-100 text-red-600 px-3 py-1 rounded-md text-[10px] flex w-max items-center gap-1 font-black uppercase">
                              <Ban size={12} /> Banned
                           </span>
                        ) : (
                      ...
                      <td className="py-4 px-6 text-right">
                        {!u.is_admin ? (
                           <button 
                             ...
                             className={`text-[10px] font-black uppercase tracking-widest px-4 py-2 rounded-xl transition shadow-sm ${u.is_banned ? 'bg-gray-900 text-white hover:bg-gray-800' : 'bg-red-50 text-red-600 border border-red-100 hover:bg-red-100'}`}
                           >
                  ...
                  {users.length === 0 && (
                    <tr>
                      <td colSpan="5" className="py-8 text-center text-sm text-gray-400 font-bold">
