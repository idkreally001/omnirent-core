...
## **Sprint 16: Geolocation, High-Trust Identity & Moderation Scaling (Pending)**
**Primary Focus:** Automating high-tier verification, implementing location-aware logistics, and decentralizing platform moderation.

### **🎯 Future Goals**

*   **1. e-Devlet (National Identity) Integration:** 
    *   Transition from manual TCKN entry to a formal **e-Devlet API** handshake.
    *   Automatically verify real-world entity existence and name-matching to absolutely eliminate synthetic/duplicate identities.
    
*   **2. Community Moderation Engine (Reporting):**
    *   **Database:** Create a structured `reports` ledger tracking reporters, targets (Items/Users), and specific abuse vectors.
    *   **Frontend UI:** Inject strict, categorizational "Report Flags" into `PublicProfile.jsx` and `ItemDetail.jsx` (e.g., *Inappropriate Photo, Prohibited Item, Fraudulent Name*).
    *   **Admin Triage:** Deploy a new "Community Reports" queue in the `AdminDashboard` allowing one-click moderation (Ban User, Force Delete Listing).
    
*   **3. Visual High-Trust Artifacts (Avatars):**
    *   Hook the existing `imageCompression.js` and Server-Side Cloudinary Signature architecture into a new `AvatarUpload` component.
    *   Allow verified users to upload/remove human profile pictures to replace legacy letter-block placeholders, enhancing peer-to-peer familiarity.

*   **4. Geospatial Awareness & Safe Zones:**
    *   **Discovery Engine:** Implement browser Geolocation API logic to inject "Near Me / XYZ km Away" sorting into the `Browse.jsx` feed.
    *   **Safety Logistics:** Develop a backend library of pre-cleared "Safe Transfer Zones" (i.e. high-traffic malls, police stations) limiting where users mathematically agree to meet for hardware handovers.

*   **5. Intelligent Memory & Personalization (Cookies):**
    *   Utilize local storage and legal Functional Cookies to synthesize an "Amazon-like" Behavioral memory.
    *   Build "Pick up where you left off" feed rails (Recently Viewed listings) and persist localized filter preferences permanently.
